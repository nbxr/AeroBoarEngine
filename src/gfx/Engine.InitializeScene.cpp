...
bool gfx::Engine::load_scene(const std::string &scene_name) {
    ...
    // Populate initial FrameGlobals (lights + camera) into the upload side
    {
        ...
        if (dst) {
            ...
            // This is the primary light "for now". Full per-scene / glTF
            ...
            // Scene lights from glTF are collected but not yet the primary
            // source (we keep them for future expansion but currently rely on
            // the global light). for (const auto& sl : renderer.lights) { ... }
            // // future work
    ...
    // Safe GLTF traversal:
    // - Only process nodes that actually reference a mesh (skip cameras,
    // lights, groups, etc.)
    //   See docs/architecture/lighting-implementation.md — glTF lights are
    //   extracted but the engine global light is the primary source for now.
    ...
    // indices.
    std::function<void(int, const glm::mat4 &)> add_mesh_node =
        [&](int node_idx, const glm::mat4 &parent_xform) {
    ...
    // Minimal glTF camera support: use author camera if present, otherwise fall
    // back to AABB framing
    if (loaded_camera.valid) {
        camera.set_from_camera_node(loaded_camera.world_transform,
                                    loaded_camera.yfov, loaded_camera.znear,
                                    loaded_camera.zfar);
